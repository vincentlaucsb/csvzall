...
const positioning_js_1 = require("./positioning.cjs");
...
class ContextMenu {
     ...
     */
    itemsSource = null;
    constructor(controller, target, options, internal = {}) {
        this.controller = controller;
        this.targets = (0, utils_js_1.normalizeTargets)(target);
        this.options = (0, utils_js_1.normalizeOptions)(options);
    ...
    openNow(input) {
        if (this.destroyed || !(0, utils_js_1.canUseDom)()) {
            return;
        }
        const context = (0, utils_js_1.createMenuContext)(input);
        ...
        const items = (0, utils_js_1.normalizeItems)((0, utils_js_1.resolveItems)(this.options.items, context));
        if (!(0, utils_js_1.hasSelectableOrLabelContent)(items)) {
            if (this.registeredWithController) {
                this.controller.clearActive(this);
            }
            return;
        ...
        this.items = items;
        this.activeIndex = (0, utils_js_1.firstSelectableIndex)(items);
        const root = (0, render_js_1.createMenuRoot)({
        ...
        (0, theme_js_1.applyTheme)(root, this.options.theme);
        (0, render_js_1.renderMenuItems)(root, {
            items: this.items,
            context,
            options: this.options,
            onItemEnter: (index) => {
                if ((0, utils_js_1.isSelectable)(this.items[index])) {
        ...
        this.root = root;
        this.unsubscribeTheme = ThemeStore_js_1.contextMenuTheme.subscribe(() => {
            if (this.root) {
                (0, theme_js_1.applyTheme)(this.root, this.options.theme);
            }
        });
    ...
    close(reason = "manual", nativeEvent) {
    ...
    update(options) {
        this.options = (0, utils_js_1.normalizeOptions)({ ...this.options, ...options });
        if (this.root) {
            (0, theme_js_1.applyTheme)(this.root, this.options.theme);
        }
    }
    ...
    positionRoot(root, input) {
        ...
        const position = (0, positioning_js_1.computeMenuPosition)({
    ...
    getPreferredPosition(input, rect) {
    ...
    onPointerMove(event) {
        const item = event.target?.closest?.("[data-popright-item]");
    ...
    onClick(event) {
        ...
        this.selectIndex(Number(item.dataset.index), event);
    }
    onKeyDown(event) {
        ...
        if (event.key === "Home") {
            event.preventDefault();
            this.activeIndex = (0, utils_js_1.firstSelectableIndex)(this.items);
            this.updateActiveDom();
            return;
        }
        if (event.key === "End") {
            event.preventDefault();
            this.activeIndex = (0, utils_js_1.lastSelectableIndex)(this.items);
            this.updateActiveDom();
            return;
        }
    ...
    moveActive(delta) {
        const selectable = (0, utils_js_1.selectableIndexes)(this.items);
    ...
    selectIndex(index, nativeEvent) {
        const item = this.items[index];
        if (!(0, utils_js_1.isSelectable)(item)) {
            return;
        }
        ...
        const event = {
            ...
            item,
            nativeEvent,
            context: this.currentContext ?? (0, utils_js_1.createMenuContext)({ triggerEvent: nativeEvent }),
    ...
    updateActiveDom() {
        ...
        (0, render_js_1.updateActiveDom)(this.root, this.items, this.activeIndex, this.options);
    }
    containsRoot(target) {
        return (!!this.root &&
    ...
    openSubmenu(index) {
        if (!this.root || !(0, utils_js_1.canUseDom)()) {
            return;
        }
        const item = this.items[index];
        ...
        const trigger = this.root.querySelector(`[data-popright-item][data-index="${index}"]`);
        if (!trigger) {
            return;
        }
        ...
        this.closeChild("manual");
        const childItems = (0, utils_js_1.normalizeItems)((0, utils_js_1.resolveItems)(item.items, this.currentContext ?? (0, utils_js_1.createMenuContext)({}))).filter(utils_js_1.isChildMenuItem);
        if (!(0, utils_js_1.hasSelectableOrLabelContent)(childItems)) {
            return;
        }
        const childMenu = new ContextMenu(this.controller, null, {
            ...this.options,
            trigger: "manual",
            placement: "target",
    ...
    closeChild(reason = "manual", nativeEvent) {
        ...
        this.childMenu = null;
        child.destroy();
        const trigger = this.root?.querySelector("[data-popright-submenu-trigger][aria-expanded='true']");
