...
public:
  BarCommand(charts::BarSpec spec,
             std::istream& input,
             std::ostream& output,
...
protected:
  int run() override {
    try {
      const auto result = charts::RenderBarCsv(
...
private:
  charts::BarSpec spec_;
  std::ostream& output_;
};

int RunBar(const charts::BarSpec& spec,
           std::istream& input,
           std::ostream& output,
           const RunOptions& options,
           const LoggerCallbacks& logger,
           RunStats& stats) {
  BarCommand cmd(spec, input, output, options, logger, stats);
  return cmd.execute();
}
