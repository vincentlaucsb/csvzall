...
set(_assets
  ...
  "/assets/modules/markdown.mjs|${VIEWER_SOURCE_DIR}/modules/markdown.mjs|application/javascript|viewer_module_markdown_mjs"
  "/assets/modules/save.mjs|${VIEWER_SOURCE_DIR}/modules/save.mjs|application/javascript|viewer_module_save_mjs"
  "/assets/modules/sql.mjs|${VIEWER_SOURCE_DIR}/modules/sql.mjs|application/javascript|viewer_module_sql_mjs"
  "/assets/modules/state.mjs|${VIEWER_SOURCE_DIR}/modules/state.mjs|application/javascript|viewer_module_state_mjs"
  ...
  "/assets/popright/dropdown.css|${POPRIGHT_SOURCE_DIR}/dropdown.css|text/css|popright_dropdown_css"
  ...
  "/assets/popright/DropdownMenu.js|${POPRIGHT_SOURCE_DIR}/DropdownMenu.js|application/javascript|popright_dropdown_menu_js"
  ...
  "/assets/popright/render/items.js|${POPRIGHT_SOURCE_DIR}/render/items.js|application/javascript|popright_render_items_js"
  "/assets/popright/render/root.js|${POPRIGHT_SOURCE_DIR}/render/root.js|application/javascript|popright_render_root_js"
  "/assets/popright/render/types.js|${POPRIGHT_SOURCE_DIR}/render/types.js|application/javascript|popright_render_types_js"
...
# Run on every build: archives/package managers may preserve older input mtimes.
# Hash the generator too, so changes to routes, chunking, or output format invalidate
# the cache. Only successful generation publishes the new fingerprint.
file(SHA256 "${CMAKE_CURRENT_LIST_FILE}" _generator_hash)
set(_fingerprint "${_generator_hash}")
...
set(_stamp "${OUTPUT_CPP}.sha256")
if(EXISTS "${OUTPUT_CPP}" AND EXISTS "${OUTPUT_HPP}" AND EXISTS "${_stamp}")
  file(READ "${_stamp}" _previous_fingerprint)
  if(_previous_fingerprint STREQUAL _fingerprint)
    return()
  endif()
endif()
...
foreach(_asset IN LISTS _assets)
  ...
  while(_offset LESS _content_len)
    ...
    string(APPEND _cpp "constexpr char k_${_symbol}_chunk_${_chunk_index}[] = R\"${_delimiter}(${_chunk})${_delimiter}\";\n")
    string(APPEND _chunk_entries "  std::string_view{k_${_symbol}_chunk_${_chunk_index}, sizeof(k_${_symbol}_chunk_${_chunk_index}) - 1},\n")
...
# Preserve output mtimes when generated bytes are unchanged (especially the header).
file(WRITE "${OUTPUT_HPP}.tmp" "${_hpp}")
file(WRITE "${OUTPUT_CPP}.tmp" "${_cpp}")
configure_file("${OUTPUT_HPP}.tmp" "${OUTPUT_HPP}" COPYONLY)
configure_file("${OUTPUT_CPP}.tmp" "${OUTPUT_CPP}" COPYONLY)
file(REMOVE "${OUTPUT_HPP}.tmp" "${OUTPUT_CPP}.tmp")
